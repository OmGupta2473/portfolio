...
import { useRef, useState, useEffect, memo } from "react"; // ADDED 'memo' here
import { motion, useScroll, useTransform, useInView, AnimatePresence, useMotionValue, useSpring, useVelocity } from "framer-motion";
import { ArrowDown, ArrowUpRight, ExternalLink, Github, X, ChevronRight, Globe } from "lucide-react"; // ADDED 'Globe'
...
const projects = [
  {
    ...
    link: "#",
    detailedDesc: "Designed a centralized knowledge retrieval system for a large-scale enterprise."
  },
  {
    id: 2,
    ...
    link: "#",
    detailedDesc: "A bespoke e-commerce solution tailored for a boutique nail salon."
  },
  {
    id: 3,
    ...
    link: "#",
    detailedDesc: "A SaaS tool allowing users to upload legal contracts and ask complex questions."
  },
  {
    id: 4,
    ...
    link: "#",
    detailedDesc: "A personal assistant app that runs a quantized LLM locally on the device for privacy."
  },
  {
    id: 5,
    title: "Risk Model",
    ...
    link: "#",
    detailedDesc: "Built a regression model to predict stock volatility based on 10 years of historical market data."
  },
  {
    id: 6,
    title: "CSV AI Agent",
    ...
    link: "#",
    detailedDesc: "An agent that converts natural language questions into Pandas code to analyze uploaded CSV files."
  },
];
...
// --- 3. PROJECT DETAIL MODAL (RESTORED MISSING COMPONENT) ---
  ...
  return (
    ...
    >
      <motion.div 
        ...
        animate={{ scale: 1, y: 0 }}
        exit={{ scale: 0.9, y: 50 }}
        className="bg-[#111] border border-white/10 w-full max-w-4xl h-[85vh] rounded-2xl overflow-hidden shadow-2xl flex flex-col relative"
        onClick={(e) => e.stopPropagation()}
      >
         <button onClick={onClose} className="absolute top-4 right-4 z-50 p-2 bg-white/10 rounded-full hover:bg-white/20 transition-colors">
         ...
         <div className="h-[40%] relative">
             ...
             <div className="absolute bottom-6 left-6 md:bottom-8 md:left-8">
                <span className="text-xs font-bold text-orange-500 uppercase tracking-widest mb-2 block">{project.category}</span>
                <h2 className="text-3xl md:text-5xl font-black text-white uppercase tracking-tighter">{project.title}</h2>
             </div>
         </div>
         ...
         <div 
           ...
           data-lenis-prevent
         >
            <p className="text-lg text-white/70 leading-relaxed mb-8">{project.longDescription}</p>
...
const CustomCursor = () => {
  ...
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 }); // Added state backup
  ...
  useEffect(() => {
    ...
    const updateMouse = (e) => {
        ...
        setMousePosition({ x: e.clientX, y: e.clientY }); // Backup for simpler logic if needed
    };

    const handleMouseOver = (e) => setIsHovering(['A', 'BUTTON', 'CANVAS', 'H3', 'P'].includes(e.target.tagName));
  ...
  return (
    <motion.div
      ...
      style={{ x: smoothX, y: smoothY, scaleX, scaleY, translateX: "-50%", translateY: "-50%" }}
        ...
        <section className="min-h-screen relative flex flex-col justify-center px-6 md:px-12 pt-24 z-10 border-b border-white/5">
          <div className="w-full grid grid-cols-1 md:grid-cols-12 gap-y-4">
            <div className="col-span-12 md:col-span-8 flex flex-col items-start leading-[0.85]">
              ...
              {/* HERO TEXT: Updated to match your specific sizing request */}
              <div className="overflow-hidden">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1, duration: 1 }} className="col-span-12 md:col-span-4 flex md:justify-end md:items-start mt-4 md:mt-2">
                {/* ALIGNMENT PARAGRAPH: Kept your manual tweak for specific alignment */}
